...
-- Stored procedured to be exceuted whenever new files comes in from the source:
...
-- command to call the store procedure:
call pdr_scd_demo();

-- To automate the execution of stored procedure, we will need to implement it using Task in snowflake
-- step1: Setup a TaskAdmin:
...
-- Creating the actual task to call the stored procedure:
...
--to get the schedule time of task:
